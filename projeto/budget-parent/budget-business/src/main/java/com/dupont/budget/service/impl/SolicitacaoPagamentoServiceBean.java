...
public class SolicitacaoPagamentoServiceBean implements SolicitacaoPagamentoService {
	...
	private void callSolicitacaoPagamentoProcess( SolicitacaoPagamento solicitacaoPagamento) {
		...
		for (DespesaSolicitacaoPagamento despesa : solicitacaoPagamento.getDespesas()) {
			...
			for(PapelUsuario papel : despesa.getCentroCusto().getArea().getPapeis())
			{
				if(papel.getPapel().getNome().startsWith("NOTAS_"))
				{
					_area.setNomePapelLancamento(papel.getPapel().getNome());
				}
				else if(papel.getPapel().getNome().startsWith("LIDER_"))
				{
					_area.setNomePapelLider(papel.getPapel().getNome());
				}
			}
