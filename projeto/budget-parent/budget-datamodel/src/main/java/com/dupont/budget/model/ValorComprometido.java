...
@NamedQueries({
	@NamedQuery(name = ValorComprometido.FIND_BY_FILTRO, query = "select v from ValorComprometido v where lower(v.acao.nome) = lower(:acao) and lower(v.centroCusto.codigo) = :centroCusto and lower(v.tipoDespesa.nome) = lower(:tipoDespesa) and v.mes = :mes")
})
